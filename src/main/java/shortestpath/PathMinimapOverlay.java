...
public class PathMinimapOverlay extends Overlay {
    ...
    public Dimension render(Graphics2D graphics) {
        ...
        if (plugin.path != null) {
            for (WorldPoint point : plugin.path) {
                ...
                drawOnMinimap(graphics, point, new Color(255, 0, 0, 255));
            }
        }

        for (Map.Entry<String, WorldMapActionInfo> entry : plugin.taskMarkers.entrySet())
        {
            if (entry.getValue().Enabled)
            {
                drawOnMinimap(graphics, entry.getValue().marker.getWorldPoint(),entry.getValue().markercolor);
            }
        }
    ...
    private void drawOnMinimap(Graphics2D graphics, WorldPoint point, Color colorToDraw) {
        WorldPoint playerLocation = client.getLocalPlayer().getWorldLocation();

        if (point == null || point.distanceTo(playerLocation) >= 50)
        {
            return;
        }
        ...
        renderMinimapRect(client, graphics, posOnMinimap, TILE_WIDTH, TILE_HEIGHT, colorToDraw);
    }
